class CardMarkup {
  ...
  card(appendContainer, title) {
    const CARD_ITEM_ELEMENT = $("<div> </div>");
    CARD_ITEM_ELEMENT.addClass(
    ...
    CARD_ITEM_ELEMENT.append(IMG_CONTAINER_ELEMENT);
    CARD_ITEM_ELEMENT.append(TITLE_ELEMENT);
    CARD_ITEM_ELEMENT.append(DETAILS_ELEMENT);
    CARD_ITEM_ELEMENT.append(LEARN_MORE_BTN_ELEMENT);

    // Append the card to the container
    appendContainer.append(CARD_ITEM_ELEMENT);
    return CARD_ITEM_ELEMENT;
  }

  cardImg() {
    const CARD_IMG_CONTAINER_ELEMENT = $("<div> </div>");
    CARD_IMG_CONTAINER_ELEMENT.addClass("rounded-t-lg overflow-hidden w-full");

    const CARD_IMG_ELEMENT = $("<img> </img>");
    CARD_IMG_ELEMENT.attr("src","https://picsum.photos/250"); // Unsplash random image
    CARD_IMG_ELEMENT.addClass("w-full h-48 object-cover"); // Added responsive image styles

    CARD_IMG_CONTAINER_ELEMENT.append(CARD_IMG_ELEMENT);
    return CARD_IMG_CONTAINER_ELEMENT;
  }

  cardTitle(title) {
    const CARD_TITLE_ELEMENT = $("<h3> </h3>");
    CARD_TITLE_ELEMENT.addClass(
      "mt-4 mb-2 text-2xl font-bold text-gray-900 dark:text-white transition-colors duration-300"
    );
    CARD_TITLE_ELEMENT.text(title);
    return CARD_TITLE_ELEMENT;
  }

  cardDetails() {
    const CARD_DETAILS_ELEMENT = $("<div> </div>");
    CARD_DETAILS_ELEMENT.addClass(
      "mb-4 text-base text-gray-700 dark:text-gray-400"
    );
    CARD_DETAILS_ELEMENT.text(
      "This is a sample description text for the card. Modify it as needed."
    );
    return CARD_DETAILS_ELEMENT;
  }

  learnMoreBtn() {
    const CARD_LEARN_MORE_BTN_ELEMENT = $("<a> </a>");
    CARD_LEARN_MORE_BTN_ELEMENT.attr('href', '#');
    CARD_LEARN_MORE_BTN_ELEMENT.addClass(
    ...
    CARD_LEARN_MORE_BTN_ELEMENT.text("Learn More");
